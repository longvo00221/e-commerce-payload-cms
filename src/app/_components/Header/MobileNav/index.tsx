...
const MobileNav: React.FC<MobileNavProps> = ({ header, user, isNavMobileOpen, setIsNavMobileOpen }) => {
    ...
    return (
        <div className={`fixed inset-0 bg-white w-full h-screen flex flex-col items-center justify-center ${isNavMobileOpen ? 'block' : 'hidden'}`}>
            ...
            <div className="flex flex-col items-center justify-center">
                <Link href="/cart" className="mt-4 rounded-full p-2 hover:bg-black/10" onClick={handleLinkClick}>
                    <p className="text-2xl">Cart</p>
                </Link>
                {user ? (
                    <Link href="/account" className="mt-4 rounded-full p-2 hover:bg-black/10" onClick={handleLinkClick}>
