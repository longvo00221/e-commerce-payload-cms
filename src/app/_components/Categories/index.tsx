...
const Categories: React.FC<CategoriesProps> = ({ categories }) => {
    const storeCategories = categories.filter(category =>category.parent && category.parent?.title === 'stores');
    const serviceCategories = categories.filter(category =>category.parent && category.parent?.title === 'services');
    useEffect(() => {
        AOS.init();
    }, [])
    return (
